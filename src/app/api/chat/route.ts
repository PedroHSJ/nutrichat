...
import { withChatGuard, incrementInteractionUsage } from '@/lib/subscription-guard';
...
async function chatHandler(request: NextRequest) {
  try {
    ...
    console.log(`[CHAT] Usando provider: ${aiProvider.name} para usuário: ${user.id}`);

    try {
      // Processar mensagem com IA
      const assistantMessage = await aiProvider.sendMessage(message, SYSTEM_PROMPT);
