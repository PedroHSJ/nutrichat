...
async function getAuthenticatedUser(request: NextRequest) {
  ...
  try {
    ...
    return {
      ...
      name:
        data.user.user_metadata?.name ?? data.user.email ?? "Usuário NutriChat",
    };
  } catch (error) {
...
async function handler(request: NextRequest) {
  try {
    ...
    if (!openAiResponse.ok) {
      ...
      console.error(
        "[chatkit/session] Erro da OpenAI ao criar sessão:",
        errorPayload,
      );
      return NextResponse.json(
    ...
    if (!clientSecret) {
      console.error(
        "[chatkit/session] Resposta inesperada ao criar sessão:",
        payload,
      );
      return NextResponse.json(
